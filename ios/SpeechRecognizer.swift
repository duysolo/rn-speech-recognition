...
actor SpeechRecognizer: ObservableObject {
  ...
  private static func getFileAudioFormat(
    ...
    if let outputEncoding = options.recordingOptions?.outputEncoding {
      ...
      default:
        print(
          "[rn-speech-recognition] Unsupported encoding: \(outputEncoding). Using default pcmFormatFloat32."
        )
      }
    }
  ...
  private func prepareFileRecognition(request: SFSpeechRecognitionRequest, url: URL) throws {
    ...
    Task.detached(priority: .userInitiated) {
      do {
        ...
        print("[rn-speech-recognition]: Audio streaming ended")
        request.endAudio()
        // await self.playBack(playbackBuffers: playbackBuffers)
      } catch {
        print("[rn-speech-recognition]: Error feeding audio file: \(error)")
        request.endAudio()
      }
    }
  }
  ...
  private func prepareMicrophoneRecognition(
    ...
    guard let audioEngine = self.audioEngine else {
      print("rn-speech-recognition: ERROR - Failed to create AVAudioEngine")
      throw RecognizerError.invalidAudioSource
    }
    ...
    guard !Self.audioInputIsBusy(audioFormat) else {
      print("rn-speech-recognition: ERROR - input is busy \(audioFormat)")
    ...
    if options.iosVoiceProcessingEnabled == true {
      do {
        ...
        print(
          "rn-speech-recognition: WARNING - Failed to set voice processing: \(error)"
        )
      }
    }
    ...
    if options.recordingOptions?.persist == true {
      guard let fileAudioFormat = Self.getFileAudioFormat(options: options, engine: audioEngine)
        ...
        print(
          "rn-speech-recognition: ERROR - Failed to create AVAudioFormat from given sample rate")
        throw RecognizerError.invalidAudioSource
      }
  ...
  private func prepareFileWriter(
    ...
    if let outputDirectory = outputDirectory {
      guard let resolvedURL = Self.resolveOutputDirectoryURL(outputDirectory) else {
        print("rn-speech-recognition: ERROR - Invalid outputDirectory: \(outputDirectory)")
        return nil
      }
      baseDir = resolvedURL
    } else {
    ...
    do {
      ...
      print("rn-speech-recognition: ERROR - Failed to create output directory: \(error)")
      return nil
    }
    ...
    guard status == noErr else {
      print("rn-speech-recognition: ERROR - Failed to create audio file: \(status)")
      audioFileRef = nil
      return nil
    }
  ...
  private static func resolveOutputDirectoryURL(_ directory: String) -> URL? {
      ...
      !scheme.isEmpty
    {
      guard url.isFileURL else {
        print("rn-speech-recognition: ERROR - Output directory must be a file URL.")
        return nil
      }
